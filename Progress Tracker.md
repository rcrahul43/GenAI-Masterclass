# Progress Tracker (Learner)

> Your personal completion checklist. Separate from authoring [`Progress.md`](Progress.md).

**Related:** [COURSE.md](COURSE.md) · [Study Plan](Study%20Plan.md) · [Dashboard](Dashboard.md) · [TOC](TABLE_OF_CONTENTS.md) · Advanced: [Master Study Roadmap](Master%20Study%20Roadmap.md)

---

## Prefer the Excel tracker

Use [`Tracker/GenAI-Masterclass-Tracker.xlsx`](Tracker/GenAI-Masterclass-Tracker.xlsx) for Status / weeks / resources.  
Markdown checkboxes below are an optional backup. Sync notes: [`Tracker/SYNC-WORKFLOW.md`](Tracker/SYNC-WORKFLOW.md).

---

## How to Score Yourself (Track D)

| Status | Meaning |
|--------|---------|
| ⬜ | Not started |
| 🔄 | In progress |
| ✅ | Track D scope done + lab shipped + Git commit |
| ⭐ | Can demo + explain to a hiring manager |

Only mark ✅ when the week’s **ship** item exists.

---

## Core Navigation

- [ ] **[COURSE.md](COURSE.md)** reviewed (primary course)
- [ ] README + Dashboard orientation
- [ ] Study Plan (Track D 48 weeks) opened
- [ ] Track **D** confirmed
- [ ] (Later) Advanced Master Study Roadmap — year 2+

---

## Track D — Block checklist (primary)

- [ ] **Block 1** Foundations (Weeks 1–8) — Hello-LLM + FastAPI
- [ ] **Block 2** LLM apps (Weeks 9–16) — Document helper
- [ ] **Block 3** Agents + RAG (Weeks 17–24) — support agent + FAQ bot
- [ ] **Block 4** Quality + safety (Weeks 25–32) — Golden set + guardrails
- [ ] **Block 5** Deploy + cost (Weeks 33–40) — Docker + ₹/query
- [ ] **Block 6** Specialize + jobs (Weeks 41–48) — Flagship + applications

---

## Advanced phase checklist (Tracks A–C / year 2+)

- [ ] Phase 0 — AI Foundations
- [ ] Phase 1 — LLM Foundations (+ DeepSeek)
- [ ] Phase 2 — Agent Fundamentals (LangGraph)
- [ ] Phase 3 — RAG
- [ ] Phase 4 — Multi-Agent + MCP depth
- [ ] Phase 5 — Voice & Multimodal
- [ ] Phase 6 — LLMOps
- [ ] Phase 7 — Fine-Tuning
- [ ] Phase 8 — Production AI Engineering
- [ ] Phase 9 — AI Security
- [ ] Phase 10 — System Design + Coding Agents + Product
- [ ] Phase 11 — Leadership & EM prep

---

## Module 00 — Foundations (Track D order)

- [ ] 00-01 GenAI From Scratch: Core Ideas  ← **start here** (Full)
- [ ] 00-02 Python for AI Engineering (Full, slow)
- [ ] 00-03 APIs for AI Engineering (Full)
- [ ] 00-04 From Rules to Agents (Skim)
- [ ] 00-05 Mathematics for AI Engineering (Light)
- [ ] 00-06 BankCo Decision-Support Warmup (Optional)

## Module 01 — LLM Engineering

- [ ] 01-01 Transformer Architecture (Intuition)
- [ ] 01-02 Tokenization & Context Windows (Full)
- [ ] 01-03 Provider SDKs (OpenAI, Claude, Gemini, DeepSeek) (Full)
- [ ] 01-04 Model Routing (LiteLLM) (Skim)
- [ ] 01-05 Inference Serving (vLLM / Ollama) (Skim in Block 5)

## Module 02 — Prompt Engineering

- [ ] 02-01 Production Prompt Engineering (Full)
- [ ] 02-02 Structured Outputs & Tool Calling (Full)

## Module 03 — Agentic Fundamentals *(before Module 04)*

- [ ] 03-01 Agent Anatomy & Loop (Full)
- [ ] 03-02 Tools, Memory & Control Flow (Full)
- [ ] 03-03 LangGraph Production Agents (Light)
- [ ] 03-04 Agentic Design Patterns *(Advanced / Block 6B)*

## Module 04 — RAG *(after Module 03)*

- [ ] 04-01 RAG Architecture (Full)
- [ ] 04-02 Chunking, Metadata & Embeddings (Full)
- [ ] 04-03 Vector DB, Hybrid Search & Reranking (Skim)
- [ ] 04-04 Advanced RAG *(Advanced / Block 6A)*

## Module 05 — Multi-Agent *(mostly Advanced year 2+)*

- [ ] 05-01 Multi-Agent Orchestration
- [ ] 05-02 Planner–Executor–Critic
- [ ] 05-03 Frameworks Comparison

## Module 06 — Conversational & Multimodal *(Advanced year 2+)*

- [ ] 06-01 Voice ASR/TTS Pipelines
- [ ] 06-02 Multimodal Agents

## Module 07 — Protocols

- [ ] 07-01 MCP
- [ ] 07-02 A2A
- [ ] 07-03 Negotiation & Async Workflows
- [ ] 07-04 MCP Production Patterns (Depth)

## Module 08 — Evaluation & LLMOps

- [ ] 08-01 Evaluation Lifecycle
- [ ] 08-02 Observability
- [ ] 08-03 Guardrails & Ship Criteria

## Module 09 — Fine-Tuning

- [ ] 09-01 PEFT / LoRA / QLoRA
- [ ] 09-02 Prompting vs RAG vs FT
- [ ] 09-03 Serving Fine-Tuned Models

## Module 10 — Production Infrastructure

- [ ] 10-01 FastAPI AI Backends
- [ ] 10-02 Docker / K8s / CI/CD
- [ ] 10-03 Cost & Latency Optimization
- [ ] 10-04 Redis / Kafka / Ray *(Advanced — skip year 1)*

## Module 11 — Security

- [ ] 11-01 OWASP LLM Top 10
- [ ] 11-02 Prompt Injection Defense

## Module 12 — Advanced

- [ ] 12-01 Research Agents
- [ ] 12-02 Text-to-SQL Agents
- [ ] 12-03 Self-Improving Agents
- [ ] 12-04 DSPy
- [ ] 12-05 AI Coding Agents (Codex, Claude Code, Cursor)
- [ ] 12-06 AI Product Thinking

---

## System Design

- [ ] ChatGPT
- [ ] Claude
- [ ] Cursor
- [ ] GitHub Copilot
- [ ] Perplexity
- [ ] Slack AI
- [ ] Notion AI
- [ ] AI Search Engine
- [ ] AI Coding Assistant
- [ ] AI Research Agent
- [ ] AI Customer Support
- [ ] AI Voice Assistant
- [ ] Multi-Agent Workflow Engine

---

## Leadership & Career

- [ ] Leading AI Teams
- [ ] Hiring AI Engineers
- [ ] AI Governance / Strategy / Metrics
- [ ] Behavioral STAR bank (≥8 stories)
- [ ] Principal/Staff Interview Guide
- [ ] EM Interview Guide

---

## Portfolio Gates (roadmap ladder)

| Gate | Criteria | Date | Done |
|------|----------|------|------|
| G1 | Customer Support Agent (Phase 2) | | [ ] |
| G2 | Internal Knowledge Bot + citations + eval (Phase 3) | | [ ] |
| G3 | Multi-agent + MCP gateway + traces (Phase 4) | | [ ] |
| G4 | Eval dashboard + K8s deploy + cost (Phases 6–8) | | [ ] |
| G5 | Security sign-off + design pack (Phases 9–10) | | [ ] |
| G6 | Capstone / Org Agent + architecture review (Phase 11) | | [ ] |

---

## Completion Summary

| Area | Done / Total |
|------|--------------|
| Modules | __ / 48 |
| System Designs | __ / 13 |
| Leadership/Career | __ / 6 |
| Portfolio Gates | __ / 6 |
| **Overall** | __ / 73 |
